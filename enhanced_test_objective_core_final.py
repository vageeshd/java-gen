import hashlib
import time
import re
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor, as_completed
from typing import List, Dict, Any, Optional, Tuple

# Import the enhanced feedback handler
from .feedback_handler import EnhancedFeedbackHandler, FeedbackAnalysis, FeedbackType, FeedbackPromptBuilder

# Import the test case modifier
from .test_case_modifier import TestCaseModifier, TestCaseModification

# Import the optimized java extractor functions
from .java_extractor import extract_java_code_blocks_with_cross_references, trim_code_context

class TestObjectiveGeneratorCore:
    """
    Enhanced test objective generator with comprehensive feedback handling,
    question answering, and multi-field support
    """
    
    def __init__(self, client, test_manager, src_dir: str, mapping_file_path: str = None, 
                 conversation_manager = None):
        self.client = client
        self.test_manager = test_manager
        self.src_dir = src_dir
        self.mapping_file_path = mapping_file_path
        self.conversation_manager = conversation_manager
        
        # Initialize enhanced feedback handling
        self.feedback_handler = EnhancedFeedbackHandler(test_manager)
        
        # Initialize test case modifier
        self.test_case_modifier = TestCaseModifier(test_manager, client)
        
        # Multi-field state management
        self.current_field_name = None
        self.field_contexts = {}  # Store contexts per field
        self.failed_fields = []
        
        # Simple conversation tracking (field-specific)
        self.conversation_history = {}  # field_name -> list of interactions
        
        # Session statistics
        self.session_stats = {
            'fields_processed': 0,
            'total_test_cases_generated': 0,
            'total_approved_cases': 0,
            'questions_answered': 0,
            'feedback_processed': 0,
            'session_start_time': datetime.now()
        }
    
    def _hash_text(self, text: str) -> str:
        """Generate hash for field identification"""
        return hashlib.sha256(text.encode("utf-8")).hexdigest()[:8]
    
    def _validate_field_data(self, field: dict) -> Tuple[bool, str]:
        """Validate field has minimum required data"""
        required_fields = ['field_name']
        
        for req_field in required_fields:
            if not field.get(req_field):
                return False, f"Missing required field: {req_field}"
        
        field_name = field.get('field_name', '').strip()
        if len(field_name) < 2:
            return False, f"Field name too short: '{field_name}'"
        
        # Validate backend_xpath if present
        backend_xpath = field.get('backend_xpath', '')
        if backend_xpath and not backend_xpath.replace('/', '').replace('_', '').replace('-', '').isalnum():
            return False, f"Invalid backend_xpath format: '{backend_xpath}'"
        
        return True, "Valid"
    
    def _add_to_conversation_history(self, field_name: str, user_input: str, ai_response: str):
        """Add interaction to field-specific conversation history"""
        if field_name not in self.conversation_history:
            self.conversation_history[field_name] = []
        
        self.conversation_history[field_name].append({
            'timestamp': datetime.now(),
            'user': user_input[:200] + "..." if len(user_input) > 200 else user_input,
            'assistant': ai_response[:300] + "..." if len(ai_response) > 300 else ai_response,
            'type': 'feedback_interaction'
        })
        
        # Keep only last 5 interactions per field to avoid context overflow
        if len(self.conversation_history[field_name]) > 5:
            self.conversation_history[field_name] = self.conversation_history[field_name][-5:]
    
    def _get_conversation_context(self, field_name: str = None) -> str:
        """Get conversation context for current or specified field"""
        target_field = field_name or self.current_field_name
        
        if not target_field or target_field not in self.conversation_history:
            return self._extract_conversation_context()  # Fallback to original method
        
        context_parts = []
        for interaction in self.conversation_history[target_field][-3:]:  # Last 3 interactions
            context_parts.append(f"USER: {interaction['user']}")
            context_parts.append(f"ASSISTANT: {interaction['assistant']}")
        
        return "\n".join(context_parts) if context_parts else ""
    
    def process_user_feedback(self, feedback_text: str, field_metadata: dict = None) -> Dict[str, Any]:
        """
        Enhanced method to process user feedback with intelligent handling.
        This is the main entry point for handling ALL types of user feedback.
        
        Returns:
        {
            'response': str,                    # Text response to user
            'feedback_type': str,              # Type of feedback detected  
            'action_taken': str,               # Action performed
            'new_test_cases': List[str],       # New TC IDs if generated
            'is_question': bool,               # Whether this was a question
            'confidence': float,               # Confidence in feedback analysis
            'field_name': str                  # Field being processed
        }
        """
        
        if not feedback_text or not feedback_text.strip():
            return {
                'error': 'Empty feedback provided',
                'response': 'Please provide your feedback or question.',
                'action_taken': 'no_action',
                'is_question': False
            }
        
        # Determine field context
        field_name = self._determine_field_context(field_metadata)
        if not field_name:
            return {
                'error': 'No field context available',
                'response': 'Please specify which field you\'re asking about or start working on a field first.',
                'action_taken': 'no_context'
            }
        
        print(f"[INFO] Processing feedback for field: {field_name}")
        print(f"[DEBUG] Feedback: {feedback_text[:100]}...")
        
        try:
            # Analyze feedback to determine intent
            feedback_analysis = self.feedback_handler.analyze_feedback(feedback_text, field_name)
            
            print(f"[INFO] Feedback analysis: {feedback_analysis.feedback_type.value} "
                  f"(confidence: {feedback_analysis.confidence:.2f}, is_question: {feedback_analysis.is_question})")
            
            # Get existing test cases for context
            existing_cases = self.test_manager.get_field_test_cases(field_name)
            conversation_context = self._get_conversation_context(field_name)
            
            # Prepare result structure
            result = {
                'feedback_type': feedback_analysis.feedback_type.value,
                'confidence': feedback_analysis.confidence,
                'field_name': field_name,
                'is_question': feedback_analysis.is_question,
                'new_test_cases': [],
                'action_taken': 'unknown'
            }
            
            # Route to appropriate handler based on feedback type
            if feedback_analysis.is_question:
                # Handle questions - provide text response only
                response_text = self._handle_question(
                    feedback_analysis, field_metadata, existing_cases, conversation_context
                )
                result.update({
                    'response': response_text,
                    'action_taken': 'answered_question'
                })
                self.session_stats['questions_answered'] += 1
                
            elif feedback_analysis.feedback_type == FeedbackType.APPROVAL:
                # Handle approval
                approval_response = self._handle_approval(feedback_analysis, existing_cases, field_name)
                result.update(approval_response)
                
            elif feedback_analysis.feedback_type == FeedbackType.REJECTION:
                # Handle rejection
                rejection_response = self._handle_rejection(
                    feedback_analysis, field_metadata, existing_cases, conversation_context
                )
                result.update(rejection_response)
                
            elif feedback_analysis.feedback_type == FeedbackType.MODIFICATION:
                # Handle modification requests - modify specific test cases
                modification_response = self._handle_test_case_modification(
                    feedback_analysis, field_metadata, existing_cases, conversation_context
                )
                result.update(modification_response)
                
            elif feedback_analysis.requires_generation:
                # Handle addition requests (generate new test cases)
                generation_response = self._handle_generation_request(
                    feedback_analysis, field_metadata, existing_cases, conversation_context
                )
                result.update(generation_response)
                
            else:
                # Handle general feedback
                result.update({
                    'response': f"I understand your feedback: \"{feedback_analysis.extracted_intent}\". How would you like me to help with the test cases for {field_name}?",
                    'action_taken': 'acknowledged_feedback'
                })
            
            # Record interaction in conversation history
            response_text = result.get('response', 'Feedback processed')
            self._add_to_conversation_history(field_name, feedback_text, response_text)
            
            self.session_stats['feedback_processed'] += 1
            
            print(f"[INFO] Feedback processed successfully: {result['action_taken']}")
            return result
            
        except Exception as e:
            print(f"[ERROR] Failed to process feedback: {str(e)}")
            return {
                'error': f'Error processing feedback: {str(e)}',
                'response': f'I encountered an error while processing your feedback. Please try again.',
                'action_taken': 'error',
                'field_name': field_name,
                'is_question': False
            }
    
    def _determine_field_context(self, field_metadata: dict = None) -> Optional[str]:
        """Determine which field we're working on"""
        if field_metadata and field_metadata.get('field_name'):
            field_name = field_metadata['field_name']
            # Update current field if different
            if field_name != self.current_field_name:
                self.current_field_name = field_name
            return field_name
        elif self.current_field_name:
            return self.current_field_name
        else:
            return None
    
    def _handle_question(self, feedback_analysis: FeedbackAnalysis, field_metadata: dict,
                        existing_cases: List[Dict], conversation_context: str) -> str:
        """Handle user questions - return text response only"""
        
        try:
            # Build question-answering prompt
            prompt = FeedbackPromptBuilder.build_question_response_prompt(
                feedback_analysis, field_metadata, existing_cases, conversation_context
            )
            
            # Call API for response
            response = self._call_api_with_retry(prompt, max_retries=2)
            
            if response and response.strip():
                # Clean response - remove any test case table artifacts
                cleaned_response = self._clean_text_response(response)
                return cleaned_response
            else:
                return f"I understand you're asking about \"{feedback_analysis.extracted_intent}\". Could you provide more specific details about what you'd like to know?"
                
        except Exception as e:
            print(f"[ERROR] Failed to answer question: {str(e)}")
            return f"I'd like to help answer your question about \"{feedback_analysis.extracted_intent}\", but I'm having trouble generating a response. Could you rephrase your question?"
    
    def _clean_text_response(self, response: str) -> str:
        """Clean API response to ensure it's text-only (no test case tables)"""
        lines = response.split('\n')
        cleaned_lines = []
        
        for line in lines:
            line = line.strip()
            # Skip lines that look like test case table headers or data
            if (line.startswith('Category') or 
                line.startswith('Functional') or
                line.startswith('TC_') or
                line.count('\t') >= 5 or
                line.count('|') >= 5):
                continue
            
            if line:  # Keep non-empty lines
                cleaned_lines.append(line)
        
        cleaned_response = '\n'.join(cleaned_lines).strip()
        
        # If we removed too much, provide a generic helpful response
        if len(cleaned_response) < 20:
            return "I understand your question. Let me know if you need clarification about any of the test cases or testing approach for this field."
        
        return cleaned_response
    
    def _handle_approval(self, feedback_analysis: FeedbackAnalysis, existing_cases: List[Dict], 
                        field_name: str) -> Dict[str, Any]:
        """Handle test case approval"""
        
        pending_cases = [case for case in existing_cases if case.get('Status') == 'pending']
        
        if not pending_cases:
            return {
                'response': "No pending test cases to approve for this field.",
                'action_taken': 'no_pending_cases'
            }
        
        # Determine which cases to approve
        if feedback_analysis.relevant_tc_ids:
            # Approve specific test cases mentioned
            tc_ids_to_approve = [
                tc_id for tc_id in feedback_analysis.relevant_tc_ids
                if any(case.get('Test Case ID') == tc_id for case in pending_cases)
            ]
        else:
            # Approve all pending cases
            tc_ids_to_approve = [case.get('Test Case ID') for case in pending_cases if case.get('Test Case ID')]
        
        if tc_ids_to_approve:
            approval_result = self.test_manager.approve_test_cases(tc_ids_to_approve)
            approved_count = len(approval_result.get('approved', []))
            
            return {
                'response': f"Great! I've approved {approved_count} test case(s) for {field_name}: {', '.join(approval_result.get('approved', []))}",
                'action_taken': 'approved_test_cases',
                'approved_cases': approval_result.get('approved', [])
            }
        else:
            return {
                'response': "I couldn't find the specific test cases you mentioned to approve.",
                'action_taken': 'no_matching_cases'
            }
    
    def _handle_rejection(self, feedback_analysis: FeedbackAnalysis, field_metadata: dict,
                         existing_cases: List[Dict], conversation_context: str) -> Dict[str, Any]:
        """Handle test case rejection and generate alternatives"""
        
        # First, reject specific cases if mentioned
        rejected_cases = []
        if feedback_analysis.relevant_tc_ids:
            rejection_result = self.test_manager.reject_test_cases(feedback_analysis.relevant_tc_ids)
            rejected_cases = rejection_result.get('rejected', [])
        
        # Then generate alternatives
        new_tc_ids = self._generate_with_analyzed_feedback(
            feedback_analysis, field_metadata, existing_cases, conversation_context
        )
        
        if rejected_cases and new_tc_ids:
            response = f"I've rejected {len(rejected_cases)} test case(s) and generated {len(new_tc_ids)} alternative(s) based on your feedback."
        elif new_tc_ids:
            response = f"I've generated {len(new_tc_ids)} alternative test case(s) based on your feedback."
        elif rejected_cases:
            response = f"I've rejected {len(rejected_cases)} test case(s). Let me know what kind of alternatives you'd like."
        else:
            response = "I understand you're not satisfied with the current test cases. Could you be more specific about what you'd like changed?"
        
        return {
            'response': response,
            'action_taken': 'handled_rejection',
            'new_test_cases': new_tc_ids,
            'rejected_cases': rejected_cases
        }
    
    def _handle_test_case_modification(self, feedback_analysis: FeedbackAnalysis, field_metadata: dict,
                                      existing_cases: List[Dict], conversation_context: str) -> Dict[str, Any]:
        """Handle requests to modify specific existing test cases"""
        
        # Identify which test cases the user wants to modify
        target_tc_ids = self.test_case_modifier.identify_target_test_cases(
            feedback_analysis.extracted_intent, existing_cases
        )
        
        if not target_tc_ids:
            return {
                'response': "I understand you want to modify test cases, but I couldn't identify which specific test case(s) you're referring to. Please mention the test case ID (e.g., TC_001) or be more specific.",
                'action_taken': 'clarification_needed'
            }
        
        modifications_created = []
        failed_modifications = []
        
        for tc_id in target_tc_ids:
            # Find the original test case data
            original_case = next((case for case in existing_cases if case.get('Test Case ID') == tc_id), None)
            
            if not original_case:
                failed_modifications.append(tc_id)
                continue
            
            # Create modified version
            modification = self.test_case_modifier.modify_test_case(
                tc_id, feedback_analysis.extracted_intent, field_metadata, original_case
            )
            
            if modification:
                modifications_created.append({
                    'original_tc_id': tc_id,
                    'temp_tc_id': modification.temp_tc_id,
                    'modification': modification
                })
            else:
                failed_modifications.append(tc_id)
        
        if modifications_created:
            # Prepare response showing what was modified
            response_parts = [f"I've created modified versions of {len(modifications_created)} test case(s):"]
            
            for mod_info in modifications_created:
                original_id = mod_info['original_tc_id']
                temp_id = mod_info['temp_tc_id']
                response_parts.append(f"- {original_id} → {temp_id} (pending your approval)")
            
            response_parts.append("\nPlease review the modifications and approve or reject them.")
            
            if failed_modifications:
                response_parts.append(f"\nCouldn't modify: {', '.join(failed_modifications)}")
            
            return {
                'response': '\n'.join(response_parts),
                'action_taken': 'created_modifications',
                'modifications_created': modifications_created,
                'pending_approval': True
            }
        else:
            return {
                'response': f"I couldn't create modified versions for the specified test cases: {', '.join(target_tc_ids)}. Please check the test case IDs or try rephrasing your modification request.",
                'action_taken': 'modification_failed'
            }
    
    def _handle_generation_request(self, feedback_analysis: FeedbackAnalysis, field_metadata: dict,
                                 existing_cases: List[Dict], conversation_context: str) -> Dict[str, Any]:
        """Handle requests for additional test cases (not modifications)"""
        
        new_tc_ids = self._generate_with_analyzed_feedback(
            feedback_analysis, field_metadata, existing_cases, conversation_context
        )
        
        if new_tc_ids:
            response = f"I've generated {len(new_tc_ids)} additional test case(s) based on your feedback: {', '.join(new_tc_ids)}"
            action_taken = "generated_additional_cases"
        else:
            response = f"I understand you want additional test cases, but I had trouble generating them. Could you provide more specific guidance?"
            action_taken = "generation_failed"
        
        return {
            'response': response,
            'action_taken': action_taken,
            'new_test_cases': new_tc_ids
        }
    
    def _generate_with_analyzed_feedback(self, feedback_analysis: FeedbackAnalysis,
                                       field_metadata: dict, existing_cases: List[Dict],
                                       conversation_context: str) -> List[str]:
        """Generate test cases using analyzed feedback"""
        
        try:
            # Build generation prompt based on feedback analysis
            prompt = FeedbackPromptBuilder.build_generation_prompt(
                feedback_analysis, field_metadata, existing_cases, conversation_context
            )
            
            # Call API with retry logic
            output = self._call_api_with_retry(prompt, max_retries=2)
            
            if output is None:
                print(f"[ERROR] Failed to generate test cases based on feedback")
                return []
            
            # Parse and store results using TestCaseManager
            field_name = field_metadata.get('field_name', 'Unknown')
            new_tc_ids = self.test_manager.parse_and_add_test_cases(
                output, 
                default_mapping=field_metadata.get('backend_xpath', ''),
                field_name=field_name
            )
            
            if new_tc_ids:
                self.session_stats['total_test_cases_generated'] += len(new_tc_ids)
            
            return new_tc_ids
            
        except Exception as e:
            print(f"[ERROR] Error generating test cases from analyzed feedback: {str(e)}")
            return []
    
    def _extract_conversation_context(self) -> str:
        """Extract relevant conversation history for current field generation"""
        
        if not self.conversation_manager:
            return ""
        
        try:
            recent_turns = self.conversation_manager.get_conversation()[-5:]  # Last 5 turns
            
            context_summary = ""
            for turn in recent_turns:
                role = turn.get('role', '')
                content = turn.get('content', '')
                
                # Truncate very long content
                if len(content) > 200:
                    content = content[:200] + "..."
                
                context_summary += f"{role.upper()}: {content}\n"
            
            return context_summary
            
        except Exception as e:
            print(f"[WARN] Could not extract conversation context: {e}")
            return ""
    
    def _build_ai_prompt(self, field_metadata: dict, java_context: str = "", conversation_context: str = "") -> str:
        """Build the complete AI prompt in FiservAI format"""
        
        # Build CONTEXT section
        context_part = f"""Field metadata and context for test case generation:

FIELD METADATA:
"""
        for key, value in field_metadata.items():
            if value:
                context_part += f"{key}: {value}\n"
        
        if java_context:
            context_part += f"""

JAVA CODE CONTEXT:
{java_context}"""
        
        if conversation_context:
            context_part += f"""

CONVERSATION CONTEXT:
{conversation_context}"""
        
        # Build QUESTION section
        question_part = """Generate test cases in EXACTLY 9 tab-separated columns:
Category | Test Case ID (blank) | Type of Validation | Test Objective | Request/Response Field | Test Steps | Expected Result | Mapping Correlation | Manual/Automation

REQUIREMENTS:
- Category: Always "Functional"
- Test Case ID: Leave blank (will be auto-assigned)
- Type of Validation: Must be one of: "Field Validation - Positive", "Field Validation - Negative", "Business Validation - Positive", "Business Validation - Negative"
- Request/Response Field: "Request" or "Response" 
- Manual/Automation: "Manual" for business validation, "Automation" for field validation
- Mapping Correlation: Use backend_xpath from field metadata
- Generate 2-4 test cases covering different validation scenarios
- Consider the conversation context for continuity

Output ONLY the test case rows, no explanations, no headers, no markdown formatting."""

        return f"====CONTEXT {context_part} ====QUESTION {question_part}"
    
    def _call_api_with_retry(self, prompt: str, max_retries: int = 3) -> Optional[str]:
        """Call API with retry logic and error handling"""
        
        for attempt in range(max_retries):
            try:
                print(f"[DEBUG] API call attempt {attempt + 1}/{max_retries}")
                response = self.client.chat_completion(prompt)
                content = response.choices[0].message.content.strip()
                
                # Check for FiservAI's "I don't know" responses
                if "sorry" in content.lower() and ("don't know" in content.lower() or "not sure" in content.lower()):
                    if attempt < max_retries - 1:
                        print(f"[WARN] AI responded with 'don't know', retrying with simpler prompt...")
                        # Create simpler fallback prompt
                        prompt = self._create_fallback_prompt(prompt)
                        time.sleep(2)
                        continue
                    else:
                        print(f"[ERROR] AI couldn't generate response after {max_retries} attempts")
                        return None
                
                return content
                
            except Exception as e:
                print(f"[ERROR] API call failed (attempt {attempt + 1}): {str(e)}")
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt  # Exponential backoff
                    print(f"[INFO] Waiting {wait_time} seconds before retry...")
                    time.sleep(wait_time)
                else:
                    print(f"[ERROR] All API retry attempts failed")
                    return None
        
        return None
    
    def _create_fallback_prompt(self, original_prompt: str) -> str:
        """Create simpler prompt when AI says 'I don't know'"""
        
        # Extract field info from original prompt
        if "FIELD METADATA" in original_prompt:
            context_part = original_prompt.split("====QUESTION")[0].replace("====CONTEXT ", "")
        else:
            context_part = "Generate basic test cases for API field validation."
        
        # Simpler question
        question = """Generate basic test cases in this simple format (tab-separated):

Functional	 	Field Validation - Positive	Test valid input	Request	Send valid data	Success expected	field/path	Automation
Functional	 	Field Validation - Negative	Test invalid input	Request	Send invalid data	Error expected	field/path	Automation

Create 2 simple test cases following this exact pattern."""
        
        return f"====CONTEXT {context_part} ====QUESTION {question}"
    
    def generate_for_field(self, field_metadata: dict) -> bool:
        """Generate test cases for a specific field with automatic context extraction"""
        
        # Validate field data
        is_valid, error_msg = self._validate_field_data(field_metadata)
        if not is_valid:
            print(f"[ERROR] Invalid field data: {error_msg}")
            return False
        
        field_name = field_metadata.get('field_name', 'Unknown')
        self.current_field_name = field_name
        
        # Store field context for session tracking
        self.field_contexts[field_name] = {
            'metadata': field_metadata,
            'generation_timestamp': datetime.now(),
            'attempts': self.field_contexts.get(field_name, {}).get('attempts', 0) + 1
        }
        
        print(f"[INFO] Generating test cases for field: {field_name}")
        
        try:
            backend_xpath = field_metadata.get("backend_xpath") or ""
            
            # Extract keywords for Java code search
            keywords = []
            if field_name:
                keywords.append(field_name)
                # Add camelCase breakdown: PostalCode -> postal, code  
                camel_parts = re.findall(r'[A-Z][a-z]*|[a-z]+', field_name)
                keywords.extend(camel_parts)
            
            if backend_xpath:
                xpath_segments = [seg.strip() for seg in backend_xpath.split('/') if len(seg.strip()) > 2]
                keywords.extend(xpath_segments)
            
            if not keywords:
                print(f"[WARN] No valid keywords found for field, using generic search")
                keywords = ["validate", "check"]
            
            print(f"[INFO] Extracting Java code for keywords: {keywords}")
            
            # Extract Java code with optimized approach
            java_context = ""
            try:
                snippets = extract_java_code_blocks_with_cross_references(
                    self.src_dir, 
                    keywords, 
                    max_depth=1,
                    mapping_file_path=self.mapping_file_path,
                    field_metadata=field_metadata
                )
                java_context = trim_code_context(
                    snippets, 
                    max_chars=2500,
                    mapping_file_path=self.mapping_file_path
                )
                
                if java_context:
                    print(f"[INFO] Extracted {len(java_context)} chars of relevant Java context")
                else:
                    print(f"[WARN] No relevant Java code found for keywords: {keywords}")
                    
            except Exception as e:
                print(f"[WARN] Java extraction failed: {str(e)}, continuing without code context")
                java_context = ""
            
            # Extract conversation context automatically
            conversation_context = self._extract_conversation_context()
            
            # Build AI prompt
            prompt = self._build_ai_prompt(field_metadata, java_context, conversation_context)
            
            # Call API with retry logic
            output = self._call_api_with_retry(prompt)
            
            if output is None:
                print(f"[ERROR] Failed to generate test cases for field: {field_name}")
                self.failed_fields.append(field_name)
                return False
            
            # Parse and store results using TestCaseManager
            new_tc_ids = self.test_manager.parse_and_add_test_cases(
                output, 
                default_mapping=backend_xpath,
                field_name=field_name  # Pass field name for multi-field support
            )
            
            if new_tc_ids:
                self.session_stats['total_test_cases_generated'] += len(new_tc_ids)
                print(f"[INFO] Generated {len(new_tc_ids)} new test cases for {field_name}")
                return True
            else:
                print(f"[WARN] No test cases parsed from AI response for {field_name}")
                return False
            
        except Exception as e:
            print(f"[ERROR] Unexpected error processing field {field_name}: {str(e)}")
            self.failed_fields.append(field_name)
            return False
    
    def generate_with_feedback(self, field_metadata: dict, feedback: str) -> bool:
        """
        Legacy method for backward compatibility - now uses enhanced feedback processing
        Returns True if new test cases were generated, False otherwise
        """
        result = self.process_user_feedback(feedback, field_metadata)
        return len(result.get('new_test_cases', [])) > 0
    
    def is_field_ready_for_completion(self, field_name: str = None) -> bool:
        """Check if current field has sufficient approved test cases"""
        
        target_field = field_name or self.current_field_name
        if not target_field:
            return False
        
        # Get field-specific approved cases from TestCaseManager
        approved_cases = self.test_manager.get_field_approved_cases(target_field)
        
        # Field is ready for completion if user has approved at least one case
        return len(approved_cases) >= 1
    
    def complete_current_field(self) -> dict:
        """Mark current field as completed and return summary"""
        
        if not self.current_field_name:
            return {'error': 'No current field to complete'}
        
        field_name = self.current_field_name
        
        # Get completion summary from TestCaseManager
        completion_data = self.test_manager.complete_field(field_name)
        
        # Update session stats
        self.session_stats['fields_processed'] += 1
        self.session_stats['total_approved_cases'] += completion_data.get('approved_count', 0)
        
        # Mark field as completed in context
        if field_name in self.field_contexts:
            self.field_contexts[field_name]['completion_time'] = datetime.now()
            self.field_contexts[field_name]['status'] = 'completed'
        
        # Reset current field
        self.current_field_name = None
        
        print(f"[INFO] Completed field: {field_name} with {completion_data.get('approved_count', 0)} approved cases")
        
        return {
            'completed_field': field_name,
            'approved_count': completion_data.get('approved_count', 0),
            'total_generated': completion_data.get('total_generated', 0),
            'completion_time': datetime.now(),
            'status': 'success'
        }
    
    def switch_to_field(self, field_name: str) -> bool:
        """Switch context to a different field"""
        
        if field_name == self.current_field_name:
            print(f"[INFO] Already working on field: {field_name}")
            return True
        
        # If there's a current field, check if it should be completed first
        if self.current_field_name:
            if self.is_field_ready_for_completion():
                print(f"[WARN] Current field {self.current_field_name} has approved cases but is not completed")
                print(f"[INFO] Consider completing it before switching to {field_name}")
        
        self.current_field_name = field_name
        print(f"[INFO] Switched to field: {field_name}")
        
        return True
    
    def get_session_summary(self) -> dict:
        """Get comprehensive summary of current session"""
        
        session_duration = datetime.now() - self.session_stats['session_start_time']
        
        # Get multi-field stats from TestCaseManager
        multi_field_stats = self.test_manager.get_multi_field_stats()
        
        completed_fields = [name for name, context in self.field_contexts.items() 
                          if context.get('status') == 'completed']
        
        return {
            'session_duration': str(session_duration),
            'current_field': self.current_field_name,
            'fields_worked_on': list(self.field_contexts.keys()),
            'completed_fields': completed_fields,
            'fields_in_progress': [name for name in self.field_contexts.keys() 
                                 if name not in completed_fields and name != self.current_field_name],
            'total_fields_processed': len(self.field_contexts),
            'failed_fields': self.failed_fields.copy(),
            'session_stats': {
                'total_test_cases_generated': self.session_stats['total_test_cases_generated'],
                'total_approved_cases': multi_field_stats.get('total_approved', 0),
                'questions_answered': self.session_stats['questions_answered'],
                'feedback_processed': self.session_stats['feedback_processed'],
                'success_rate': len(completed_fields) / len(self.field_contexts) if self.field_contexts else 0
            },
            'multi_field_breakdown': multi_field_stats
        }
    
    def export_all_completed_fields(self, output_file: str) -> bool:
        """Export all completed fields to single Excel file with sequential TC IDs"""
        
        try:
            success = self.test_manager.export_multi_field_session(output_file)
            
            if success:
                completed_fields = [name for name, context in self.field_contexts.items() 
                                  if context.get('status') == 'completed']
                print(f"[INFO] Successfully exported {len(completed_fields)} completed fields to {output_file}")
            
            return success
            
        except Exception as e:
            print(f"[ERROR] Failed to export multi-field session: {str(e)}")
            return False
    
    def get_generation_summary(self) -> Dict[str, Any]:
        """Get summary of generation results (backward compatibility)"""
        
        session_summary = self.get_session_summary()
        
        return {
            'total_test_cases': session_summary['session_stats']['total_test_cases_generated'],
            'failed_fields': self.failed_fields.copy(),
            'success_rate': session_summary['session_stats']['success_rate'] * 100,
            'fields_processed': session_summary['total_fields_processed']
        }
    
    # Bulk generation method (keep existing implementation for compatibility)
    def bulk_generate(self, fields: List[dict], batch_size: int = 5, max_workers: int = 6) -> Dict[str, Any]:
        """Bulk generation with comprehensive error handling (existing implementation)"""
        
        print(f"[INFO] Starting bulk generation for {len(fields)} fields")
        
        # Validate all fields first
        valid_fields = []
        invalid_fields = []
        
        for field in fields:
            is_valid, error_msg = self._validate_field_data(field)
            if is_valid:
                valid_fields.append(field)
            else:
                invalid_fields.append({
                    'field': field.get('field_name', 'unknown'),
                    'error': error_msg
                })
                print(f"[WARN] Skipping invalid field: {error_msg}")
        
        if not valid_fields:
            print("[ERROR] No valid fields to process")
            return {
                'success': False,
                'processed': 0,
                'failed': len(fields),
                'invalid_fields': invalid_fields,
                'error': 'No valid fields found'
            }
        
        print(f"[INFO] Processing {len(valid_fields)} valid fields in batches of {batch_size}")
        
        total_processed = 0
        total_failed = 0
        
        # Process each field individually using the single field method
        for field in valid_fields:
            field_name = field.get('field_name', 'unknown')
            
            try:
                success = self.generate_for_field(field)
                if success:
                    # Auto-approve all generated cases in bulk mode
                    field_cases = self.test_manager.get_field_test_cases(field_name)
                    pending_cases = [case for case in field_cases if case.get('status') == 'pending']
                    
                    if pending_cases:
                        tc_ids = [case.get('Test Case ID') for case in pending_cases if case.get('Test Case ID')]
                        self.test_manager.approve_test_cases(tc_ids)
                    
                    # Complete field automatically in bulk mode
                    self.complete_current_field()
                    total_processed += 1
                else:
                    total_failed += 1
                    
            except Exception as e:
                print(f"[ERROR] Bulk processing failed for field {field_name}: {str(e)}")
                total_failed += 1
        
        # Generate summary
        success_rate = (total_processed / len(valid_fields)) * 100 if valid_fields else 0
        
        summary = {
            'success': total_processed > 0,
            'processed': total_processed,
            'failed': total_failed,
            'success_rate': round(success_rate, 2),
            'invalid_fields': invalid_fields,
            'total_test_cases': self.session_stats['total_test_cases_generated']
        }
        
        print(f"\n[INFO] Bulk generation completed:")
        print(f"  - Processed: {total_processed}/{len(valid_fields)} fields ({success_rate:.1f}%)")
        print(f"  - Generated: {summary['total_test_cases']} test cases")
        
        return summary
    
    def approve_modification(self, temp_tc_id: str) -> Dict[str, Any]:
        """
        Approve a pending test case modification
        """
        try:
            success = self.test_case_modifier.approve_modification(temp_tc_id)
            
            if success:
                modification = self.test_case_modifier.pending_modifications.get(temp_tc_id)
                original_tc_id = modification.original_tc_id if modification else "unknown"
                
                return {
                    'success': True,
                    'message': f"Successfully approved modification for {original_tc_id}",
                    'original_tc_id': original_tc_id,
                    'action_taken': 'modification_approved'
                }
            else:
                return {
                    'success': False,
                    'message': f"Failed to approve modification {temp_tc_id}",
                    'action_taken': 'approval_failed'
                }
                
        except Exception as e:
            return {
                'success': False,
                'message': f"Error approving modification: {str(e)}",
                'action_taken': 'approval_error'
            }
    
    def reject_modification(self, temp_tc_id: str) -> Dict[str, Any]:
        """
        Reject a pending test case modification
        """
        try:
            success = self.test_case_modifier.reject_modification(temp_tc_id)
            
            if success:
                return {
                    'success': True,
                    'message': f"Rejected modification {temp_tc_id}",
                    'action_taken': 'modification_rejected'
                }
            else:
                return {
                    'success': False,
                    'message': f"Failed to reject modification {temp_tc_id}",
                    'action_taken': 'rejection_failed'
                }
                
        except Exception as e:
            return {
                'success': False,
                'message': f"Error rejecting modification: {str(e)}",
                'action_taken': 'rejection_error'
            }
    
    def get_pending_modifications(self, field_name: str = None) -> List[Dict[str, Any]]:
        """
        Get all pending modifications for review
        """
        target_field = field_name or self.current_field_name
        return self.test_case_modifier.get_pending_modifications(target_field)
    
    def show_modification_comparison(self, temp_tc_id: str) -> str:
        """
        Show detailed comparison between original and modified test case
        """
        return self.test_case_modifier.display_modification_comparison(temp_tc_id)
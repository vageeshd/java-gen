...
def chat_mode_with_auto_export(generator: TestObjectiveGeneratorCore, fields: List[dict]) -> bool:
    ...
    while True:
        field_input = input(f"\n📝 Enter field name (or 'list'/'exit'): ").strip()
        ...
        if not field:
            ...
            if matches:
                if len(matches) == 1:
                    field = matches[0]
                    print(f"🔍 Found partial match: {field.get('field_name')}")
                else:
                ...
                print(f"❌ Field '{field_input}' not found. Type 'list' to see available fields.")
                continue
        
        # Generate test cases for the field
        print(f"\n🔄 Generating test cases for: {field.get('field_name')}")
        print("-" * 40)
        
        success = generator.generate_for_field(field)
        
        if success:
            print(f"✅ Successfully generated test cases for {field.get('field_name')}")
            generated_any = True
            ...
            print(f"❌ Failed to generate test cases for {field.get('field_name')}")
...
def bulk_mode_with_error_handling(generator: TestObjectiveGeneratorCore, fields: List[dict], out_file: str) -> bool:
    ...
    if summary['batch_failures']:
        print(f"\n❌ BATCH FAILURES ({len(summary['batch_failures'])}):")
    ...
    if summary['invalid_fields']:
        print(f"\n⚠️  INVALID FIELDS ({len(summary['invalid_fields'])}):")
